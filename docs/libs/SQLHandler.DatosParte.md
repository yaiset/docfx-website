# <a id="SQLHandler_DatosParte"></a> Class DatosParte
...
```csharp
public class DatosParte : UserControl, IAnimatable, IFrameworkInputElement, IInputElement, ISupportInitialize, IQueryAmbient, IAddChild
```
...
### <a id="SQLHandler_DatosParte__ctor"></a> DatosParte\(\)

Inicializa una nueva instancia de la clase DatosParte.
...
### <a id="SQLHandler_DatosParte_AddDatabaseConnection_SQLHandler_ConectorSQL_"></a> AddDatabaseConnection\(ConectorSQL\)

Agrega una conexión a la base de datos.

```csharp
public void AddDatabaseConnection(ConectorSQL db)
```

#### Parameters

`db` [ConectorSQL](SQLHandler.ConectorSQL.md)

El objeto ConectorSQL que representa la conexión a la base de datos.

### <a id="SQLHandler_DatosParte_AddDatosParte_SQLHandler_ParteTrabajo_"></a> AddDatosParte\(ParteTrabajo\)

Agrega el objeto `ParteTrabajo` dado a la propiedad `Datos` y actualiza los valores de los elementos de la interfaz de usuario en consecuencia.

```csharp
public void AddDatosParte(ParteTrabajo Parte)
```
...
El objeto `ParteTrabajo` que se va a agregar.

### <a id="SQLHandler_DatosParte_AddIdentificacion_System_Int32_"></a> AddIdentificacion\(int\)

Agrega el peso dado a la suma de pesos y actualiza el número total de pesajes.
También actualiza los campos de texto que muestran el volumen total y el número
total de pesajes.

```csharp
public void AddIdentificacion(int Peso)
```
...
### <a id="SQLHandler_DatosParte_BindHandler_System_Windows_RoutedEventHandler_"></a> BindHandler\(RoutedEventHandler\)

Vincula un RoutedEventHandler al evento Click del control BotonCerrar.

```csharp
public void BindHandler(RoutedEventHandler handler)
```

#### Parameters

`handler` [RoutedEventHandler](https://learn.microsoft.com/dotnet/api/system.windows.routedeventhandler)

El RoutedEventHandler que se va a vincular.

### <a id="SQLHandler_DatosParte_CreateBindings"></a> CreateBindings\(\)

Crea las conexiones entre los elementos de la interfaz de usuario y los valores de datos.
