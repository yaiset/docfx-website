# <a id="Zonas_ZonaEntrada"></a> Class ZonaEntrada
...
```csharp
public class ZonaEntrada : UserControl, IAnimatable, IFrameworkInputElement, IInputElement, ISupportInitialize, IQueryAmbient, IAddChild
```
...
### <a id="Zonas_ZonaEntrada__ctor"></a> ZonaEntrada\(\)

Inicializa una nueva instancia de la clase ZonaEntrada.
...
### <a id="Zonas_ZonaEntrada_AntenaConectada"></a> AntenaConectada

Obtiene o establece un valor que indica si la antena está conectada.

```csharp
public bool AntenaConectada
```
...
### <a id="Zonas_ZonaEntrada_BasculaConectada"></a> BasculaConectada

Obtiene o establece un valor que indica si la báscula está conectada.

```csharp
public bool BasculaConectada
```
...
### <a id="Zonas_ZonaEntrada_IsWorkingThreadAntena"></a> IsWorkingThreadAntena

Obtiene o establece un valor que indica si el subproceso de antena está en funcionamiento.

```csharp
public bool IsWorkingThreadAntena
```
...
### <a id="Zonas_ZonaEntrada_IsWorkingThreadEstadoZona"></a> IsWorkingThreadEstadoZona

Obtiene o establece un valor que indica si el subproceso de estado de la zona está en funcionamiento.

```csharp
public bool IsWorkingThreadEstadoZona
```
...
### <a id="Zonas_ZonaEntrada_IsWorkingThreadVariablesPlc"></a> IsWorkingThreadVariablesPlc

Obtiene o establece un valor que indica si el subproceso de variables PLC está en funcionamiento.

```csharp
public bool IsWorkingThreadVariablesPlc
```
...
### <a id="Zonas_ZonaEntrada_PlcConectado"></a> PlcConectado

Obtiene o establece un valor que indica si el PLC está conectado.

```csharp
public bool PlcConectado
```
...
### <a id="Zonas_ZonaEntrada_AnomaliaSalida"></a> AnomaliaSalida

Obtiene o establece un valor que indica si hay una anomalía de salida.

```csharp
public bool AnomaliaSalida { get; set; }
```
...
### <a id="Zonas_ZonaEntrada_ContadorCiclos"></a> ContadorCiclos

Obtiene o establece el contador de ciclos.
...
### <a id="Zonas_ZonaEntrada_ExisteRegistroAnterior"></a> ExisteRegistroAnterior

Obtiene o establece un valor que indica si existe un registro anterior.

```csharp
public bool ExisteRegistroAnterior { get; set; }
```
...
### <a id="Zonas_ZonaEntrada_HayPaletEnFinal"></a> HayPaletEnFinal

Propiedad que indica si hay un pallet en la posición final de la zona.

```csharp
public bool HayPaletEnFinal { get; set; }
```
...
### <a id="Zonas_ZonaEntrada_IdZona"></a> IdZona

Obtiene o establece el identificador de la zona.
...
### <a id="Zonas_ZonaEntrada_NecesitaReseteo"></a> NecesitaReseteo

Obtiene o establece un valor que indica si se necesita un restablecimiento.

```csharp
public bool NecesitaReseteo { get; set; }
```
...
### <a id="Zonas_ZonaEntrada_PesajeCompletado"></a> PesajeCompletado

Obtiene o establece un valor que indica si el pesaje se ha completado.

```csharp
public bool PesajeCompletado { get; set; }
```
...
### <a id="Zonas_ZonaEntrada_PresenciaSalida"></a> PresenciaSalida

Obtiene o establece un valor que indica si hay presencia de salida.

```csharp
public bool PresenciaSalida { get; set; }
```
...
### <a id="Zonas_ZonaEntrada_TransponderEnviado"></a> TransponderEnviado

Obtiene o establece un valor que indica si el transpondedor se ha enviado.

```csharp
public bool TransponderEnviado { get; set; }
```
...
### <a id="Zonas_ZonaEntrada_AsociarParteZona_SQLHandler_ParteTrabajo_"></a> AsociarParteZona\(ParteTrabajo\)

Método que asocia un parte de trabajo a la zona.

```csharp
public void AsociarParteZona(ParteTrabajo DatosParte)
```
...
Los datos del parte de trabajo.

### <a id="Zonas_ZonaEntrada_AsociarPlc_PLCs_PLCLegacy_"></a> AsociarPlc\(PLCLegacy\)

Función que asocia el Plc a la zona e inicia el modo buffer.

```csharp
public void AsociarPlc(PLCLegacy PlcAsociado)
```
...
### <a id="Zonas_ZonaEntrada_CargarConfiguracionZona_System_Int32_SQLHandler_ConectorSQL_"></a> CargarConfiguracionZona\(int, ConectorSQL\)

Método que carga la configuración de la zona desde la base de datos.

```csharp
public void CargarConfiguracionZona(int IdZona, ConectorSQL dbconn)
```
...
La conexión SQL con la base de datos.

### <a id="Zonas_ZonaEntrada_ConectarAntena"></a> ConectarAntena\(\)

Método que conecta la antena RFID con la zona y suscribe el evento de transponder reconocido.

```csharp
public void ConectarAntena()
```

### <a id="Zonas_ZonaEntrada_ConectarBascula"></a> ConectarBascula\(\)

Método que conecta la báscula con la zona y suscribe los eventos de detección de peso y pesaje estable.

```csharp
public void ConectarBascula()
```

### <a id="Zonas_ZonaEntrada_DesconectarBascula"></a> DesconectarBascula\(\)

Método que desconecta la báscula de la zona.

```csharp
public void DesconectarBascula()
```

### <a id="Zonas_ZonaEntrada_EsperarTagValido"></a> EsperarTagValido\(\)

Método asíncrono que espera a que se reconozca un tag válido por la antena RFID.

```csharp
public Task EsperarTagValido()
```
...
Una tarea que se completa cuando se reconoce el tag o se produce una excepción.

### <a id="Zonas_ZonaEntrada_HandleTransponderReconocido_System_Object_System_Collections_Generic_HashSet_System_String__"></a> HandleTransponderReconocido\(object, HashSet<string\>\)

(Actualmente no funcional) Método que maneja el evento de transponder reconocido por la antena RFID .

```csharp
public void HandleTransponderReconocido(object sender, HashSet<string> Transponders)
```
...
`Transponders` [HashSet](https://learn.microsoft.com/dotnet/api/system.collections.generic.hashset\-1)<[string](https://learn.microsoft.com/dotnet/api/system.string)\>

El conjunto de tags RFID que se reconocen.

### <a id="Zonas_ZonaEntrada_IniciarHilosPlc"></a> IniciarHilosPlc\(\)

Método que inicia los hilos que se encargan de comunicarse con el PLC.

```csharp
public void IniciarHilosPlc()
```

### <a id="Zonas_ZonaEntrada_LeerPesoBascula_System_Object_System_Int32_"></a> LeerPesoBascula\(object, int\)

Método que lee el peso de la báscula cuando se detecta un peso.

```csharp
public void LeerPesoBascula(object sender, int Peso)
```
...
### <a id="Zonas_ZonaEntrada_LeerPesoEstableBascula_System_Object_System_Int32_"></a> LeerPesoEstableBascula\(object, int\)

Método que lee el peso de la báscula cuando se estabiliza el pesaje.

```csharp
public void LeerPesoEstableBascula(object sender, int Peso)
```
...
El peso estabilizado.

### <a id="Zonas_ZonaEntrada_ProcesarVariablePlc_PLCs_EstructurasPLC_DatosPLC_"></a> ProcesarVariablePlc\(DatosPLC\)

Método que procesa una variable del PLC que se recibe directamente.

```csharp
public void ProcesarVariablePlc(EstructurasPLC.DatosPLC Data)
```

#### Parameters

`Data` EstructurasPLC.DatosPLC

Los datos del PLC que se reciben.

### <a id="Zonas_ZonaEntrada_StartHiloAntena"></a> StartHiloAntena\(\)

Método que inicia el hilo que se encarga de leer el inventario de la antena RFID cada cierto tiempo.

```csharp
public void StartHiloAntena()
```

### <a id="Zonas_ZonaEntrada_StartHiloBascula"></a> StartHiloBascula\(\)

Método que inicia el hilo que se encarga de enviar el comando de lectura a la báscula cada cierto tiempo.

```csharp
public void StartHiloBascula()
```

### <a id="Zonas_ZonaEntrada_StartZona"></a> StartZona\(\)

Método que inicia la zona y sus componentes si está activa.

```csharp
public void StartZona()
```
